...
# bljython execute_change_detection_window.jy -g "/Change Track" -t "Change Template" -s "/Snap Group" -a "/Audit Group"  -u "/Audit Group 2"
#
...
if [ "${HOST_OS}" = "WindowsNT" ]
	then
	echo "Calling Jython"
	#echo "cmd /c bljython ${BLADE_PATH_WIN}\share\sensors\execute_change_detection_window.jy  -g "${TG}" -t "${TN}" -s "${SG}" -a "${AG}" -u "${UG}" -n "${NH}" -d "${DB}""
	nexec $HOST cmd /c "bljython ${BLADE_PATH_WIN}\share\sensors\execute_change_detection_window.jy  -g "${TG}" -t "${TN}" -s "${SG}" -a "${AG}"  -u "${UG}"  -n "${NH}" -d "${DB}""
	EXIT_CODE="$?"
	...
	echo "Calling Jython"
	#echo "sh -c ${BLADE_PATH_LX}/bin/bljython ${BLADE_PATH_LX}/share/sensors/execute_change_detection_window.jy -g "${TG}" -t "${TN}" -s "${SG}" -a "${AG}"  -u "${UG}"  -n "${NH}" -d "${DB}""
	nexec $HOST sh -c "${BLADE_PATH_LX}/bin/bljython ${BLADE_PATH_LX}/share/sensors/execute_change_detection_window.jy -g "${TG}" -t "${TN}" -s "${SG}" -a "${AG}"  -u "${UG}"  -n "${NH}" -d "${DB}""
	EXIT_CODE="$?"
	if [ "$EXIT_CODE" -ne "0" ]; then
		exit 1
	fi
